...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.matclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.matclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active matclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
    ...
    const pidFile = path.join(projectRoot, 'matclaw.pid');
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'matclaw', 'mount-allowlist.json'),
    )
  ) {
